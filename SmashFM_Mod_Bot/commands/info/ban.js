  ...
  module.exports = {
    ...
    run: async (client, interaction, args) => {
      ...
      if(isMod) {
        ...
        try {
            user.send(`You have been banned from **${interaction.guild.name}** by **${interaction.user.username}#${interaction.user.discriminator}** for **${reason}**.`)
        } catch (err) { 
        ...
        interaction.followUp({content: `**${user.username}#${user.discriminator}** was banned from **${interaction.guild.name}**.`}).then(msg => {
          setTimeout(() => {
